...
export default function EditValuationPage({ params }: { params: Promise<{ id: string; valId: string }> }) {
  ...
  useEffect(() => {
    const fetchValuation = async () => {
      try {
        const res = await fetch(`/api/assets/${resolvedParams.id}/valuations/${resolvedParams.valId}`);
        const json = await res.json();
        if (json.success) {
          setAsset(json.data.asset);
          
          let displayValue = json.data.valuation.value;
          if (json.data.asset.mode === "INVESTING" && !json.data.asset.isNominal) {
            const qty = json.data.asset.quantity || 1;
            if (json.data.asset.type === "SAHAM") {
              displayValue = displayValue / (qty * 100);
            } else {
              displayValue = displayValue / qty;
            }
          }
          
          reset({
            value: displayValue,
            recordedAt: new Date(json.data.valuation.recordedAt),
            notes: json.data.valuation.notes || "",
          });
        } else {
          toast.error("Gagal memuat detail valuasi");
    ...
    fetchValuation();
  }, [resolvedParams.id, resolvedParams.valId, router, reset]);

  const onSubmit = async (values: z.infer<typeof createValuationSchema>) => {
    try {
      setIsSubmitting(true);
      const response = await fetch(`/api/assets/${resolvedParams.id}/valuations/${resolvedParams.valId}`, {
        method: "PUT",
